...
int main() 
{
    // Input 4x4 RGBA pixel block
    uint8_t block[4 * 4 * 4];
    std::cout << "Please input 16 pixels, each pixel has 4 bytes (R G B A), total 64 integers (0-255):\n";
    for (int i = 0; i < 64; ++i) 
    {
        int v;
        std::cin >> v;
    ...
    {
        std::cout << "Compression result (16 bytes): ";
    ...
    {
        std::cout << "Decompression result (16 pixels, 4 bytes per pixel):\n";
        for (int i = 0; i < 16; ++i) {
            std::cout << "Pixel " << i << ": ";
